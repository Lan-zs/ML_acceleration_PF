...
def onestep_training(
    ...
    for epoch in range(num_epochs):
        ...
        for current_phi, next_phi, temp_data in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}"):
            ...
            inputs = torch.cat((current_phi.unsqueeze(1), temp_data.unsqueeze(1)), dim=1)
        ...
        with torch.no_grad():
            for i, test_loader in enumerate(test_loaders):
                ...
                for current_phi, next_phi, temp_data in tqdm(test_loader, desc=f"Epoch {epoch + 1}/{num_epochs}"):
                    # Move data to GPU
                    current_phi = current_phi.cuda()
                    next_phi = next_phi.cuda()
                    temp_data = temp_data.cuda()
